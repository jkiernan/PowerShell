                ...
                If ($Anime -eq $Folder) { # $Anime needs to match a folder under \\JPK-NAS2\TV_Shows
                    ...
                    If (Test-Path "$TvDir\$Folder") { # Verify Anime has a folder under \\JPK-NAS2\TV_Shows 
                        ...
                        foreach ($AnimeSeason in $SeasonIndex) {
                            ...
                            If ($EpisodeName -match $SeasonName) { # If name matches season - e.g. "'Shingeki no Kyojin S3 - 38.mkv' matches 'Shingeki no Kyojin S3'"
                                # Change $TvDir\$Folder to $TvDir\$Folder\$Season
                                If(!(Test-Path -Path "$TvDir\$Folder\$SeasonFolder\$NewEpisodeName")) { # Verify if \\JPK-NAS\TV_Shows\$Anime\$Season\$Episode exists
